/*
GNEWS CropComp v9.4 - PR�-COMPOSI��O E REDIMENSIONAMENTO INTELIGENTES
...
ATUALIZA��O:
- CORRE��O DEFINITIVA (CR�TICO): L�gica de tempo e posicionamento do Crop Inteligente
  foi completamente reescrita para ser expl�cita e robusta, garantindo que a nova
  pr�-composi��o ocupe o lugar exato da camada original na timeline.
- MANTIDO: Todas as outras melhorias de estabilidade e interface das vers�es recentes.
*/
...
function launchCropComp(thisObj) {
    ...
    // Fun��es auxiliares (sem altera��o)
    ...
    CropComp.buildUI = function (thisObj) {
        ...
        var precompControls = precompTab.add('panel'); precompControls.text = "Configura��es do Crop"; precompControls.alignChildren = ['fill', 'top']; precompControls.spacing = 8; precompControls.margins = 15;
        ...
        this.ui.createHeight.helpTip = "Altura desejada para a nova comp."; this.ui.createLockAspect = createControls.add("checkbox { text: 'Manter Propor��o', value: false, alignment:['fill','center'] }");
        this.ui.createLockAspect.helpTip = "Mantem proporcao ao alterar largura/altura."; var createTimeGroup = createControls.add("group { orientation: 'row' }"); this.ui.createFps = createTimeGroup.add("group { o:'row', s: StaticText { text: 'FPS:' }, e: EditText { text: '29.97', characters:6 } }").e;
        this.ui.createFps.helpTip = "Define a taxa de quadros."; this.ui.createDuration = createTimeGroup.add("group { o:'row', s: StaticText { text: '  Dura��o:' }, e: EditText { text: '10', characters:6 } }").e;
        ...
        this.ui.resizeHeight.helpTip = "Nova altura em pixels."; this.ui.resizeLockAspect = resizeControls.add("checkbox { text: 'Manter Propor��o', value: false, alignment:['fill','center'] }");
        this.ui.resizeLockAspect.helpTip = "Bloqueia proporcao durante ajustes."; this.ui.resizeScaleContent = resizeControls.add("checkbox { text: 'Redimensionar conte�do para ajustar', value: true, alignment:['fill','center'] }");
        ...
        this.ui.resizeFps.helpTip = "Valor usado quando alterar FPS estiver ativo."; var resizeDurGroup = resizeControls.add("group { orientation: 'row' }"); this.ui.resizeChangeDurationCheck = resizeControls.add("checkbox { text: 'Alterar Dura��o para:' }");
        ...
        this.ui.resizeDuration.helpTip = "Nova duracao em segundos."; this.ui.resizeBtn = resizeControls.add("button { text: 'Redimensionar Comp(s)', alignment:['fill','center'] }");
    ...
    // As fun��es auxiliares e as outras abas n�o precisam de altera��o
    ...
    CropComp.runLayerMode = function () {
        ...
        if (!comp || !(comp instanceof CompItem)) { this.updateStatus("Abra uma composi��o primeiro.", "error"); return; }
            ...
            if (convertIndividually) {
                ...
                for (var i = layersToProcess.length - 1; i >= 0; i--) {
                    ...
                    if (!bounds || bounds.width <= 0 || bounds.height <= 0) {
                        self.updateStatus("N�o foi poss�vel calcular os limites da layer '" + currentLayer.name + "'.", "warning");
                        continue;
                    }
                    ...
                    if (precompLayer) {
                        // CORRE��O DEFINITIVA: L�gica de 3 passos para sincronia de tempo.
                        if (newDuration > 0) {
                            // 1. Ajusta a dura��o da composi��o interna.
                            precompItem.duration = newDuration;
                            // 2. Desloca as camadas internas para come�ar em tempo 0.
                ...
                if (!bounds || bounds.width <= 0 || bounds.height <= 0) { throw new Error("N�o foi poss�vel calcular os limites das layers."); }
                ...
                self.updateStatus("Crop em grupo '" + precompName + "' conclu�do.", "success");
            }

        } catch (error) { this.updateStatus(error.message, "error"); } finally { app.endUndoGroup(); }
    };
    
    // O resto das fun��es (calculateBounds, runProjectMode, etc) permanecem sem altera��o
