...
function getFolderStructureAsData(rootFolder, fileFilter, categoryName, progress) {
    ...
    var jornaisExclusions = [
        'ICONES', 'ILUSTRACOES', 'FOTOS PARA ABERTURAS', 'BAGUNCA ALHEIA', '_OLD', 
        'BACKUP', 'VERSOES ANTERIORES', 'PARA_SCRIPT', '_PREVIEWS', '_BASES_TEMATICAS', 
        'TESTE', 'REFERENCIAS', 'OLD'
    ];
...
function d9ProdFoldersDialog(prodArray) {
    ...
    function checkPathStatus(pathStr, categoryName) {
        ...
        try {
            cacheFile.open('r'); var content = cacheFile.read(); cacheFile.close();
    ...
    function addPathLine(parentGrp, pathTxt, categoryName) {
        ...
        try { setFgColor(pathLab, normalColor2); } catch (e) {} // Cor neutra inicial
        ...
        testBtn.onClick = function() {
            ...
            try {
                ...
                var masterCacheData = {};
                if (cacheFile.exists) {
                    try { cacheFile.open('r'); masterCacheData = JSON.parse(cacheFile.read()); cacheFile.close(); } catch(e) { masterCacheData = {}; }
                }
    ...
    D9T_CONFIG_w.onShow = function() {
        ...
        for (var i = 0; i < pathLabsToCheck.length; i++) {
            ...
            var color = checkPathStatus(item.label.properties.pathValue, item.category);
