...
function d9TemplateDialog() {
	...
	var scriptVersion = '3.7'; // Versão incrementada
    ...
    // ALTERAÇÃO: Grupo da direita reorganizado conforme solicitado
	...
	function updateItemCounter() {
		var count = 0; function countItemsInNode(node) { var nodeCount = 0; if (!node.items) return 0; for (var i = 0; i < node.items.length; i++) { var item = node.items[i]; if (item.type === 'item') { nodeCount++; } else if (item.type === 'node') { nodeCount += countItemsInNode(item); } } return nodeCount; } count = countItemsInNode(templateTree); itemCounterLab.text = count + (count === 1 ? ' item' : ' itens');
	}
    ...
    function expandAllNodes(tree) { if (!tree || !tree.items) return; tree.visible = false; try { function expandRecursive(node) { for (var i = 0; i < node.items.length; i++) { var item = node.items[i]; if (item.type === 'node') { item.expanded = true; if (item.items && item.items.length > 0) { expandRecursive(item); } } } } expandRecursive(tree); } finally { tree.visible = true; } }
