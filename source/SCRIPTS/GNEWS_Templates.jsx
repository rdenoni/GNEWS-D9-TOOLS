...
// ADIÇÃO: Versão iterativa e segura da função flattenData para evitar "Stack overrun".
...
function d9TemplateDialog() {
  // ALTERAÇÃO: Exibe a janela de carregamento imediatamente
  ...
  try {
    ...
    userConfigFile = new File(
      centralConfigFolder.fullName + "/TEMPLATES_config.json"
    );
  } catch (e) {
    userConfigFile = null;
  }
  var artesData = null;
  try {
    var artesDataFile = new File(
      scriptMainPath + "source/libraries/dados_json/DADOS_artes_gnews.json"
    );
  ...
  function setFgColor(element, hexColor) {
    try {
      ...
      element.graphics.foregroundColor = element.graphics.newPen(
        pType,
        color,
        1
      );
  ...
  var infoBtn;
  if (
    typeof themeIconButton === "function" &&
    typeof D9T_INFO_ICON !== "undefined"
  ) {
  ...
  var itemCounterLab = templatesHeaderOptionsGrp.add(
    "statictext",
    undefined,
    "",
    { justify: "right" }
  ...
  flatViewCheckbox.helpTip =
    "Marque para exibir todos os arquivos em uma lista plana, sem a hierarquia de pastas.";
  ...
  if (
    typeof themeIconButton === "function" &&
    typeof D9T_ATUALIZAR_ICON !== "undefined"
  ) {
  ...
  if (
    typeof themeIconButton === "function" &&
    typeof D9T_PASTA_ICON !== "undefined"
  ) {
  ...
  function generateCodigoFromTemplate(templateName) {
    try {
      ...
      if (words.length >= 3) {
        ...
        codigo =
          firstTwoLetters +
          letterAfterFirstSpace +
          letterAfterSecondSpace +
          currentYear;
  ...
  function updateArteInfo() {
    try {
      ...
      if (
        ...
        var servidorDestino = determineServidorDestino(
          templateNameWithExt,
          templatePath
        );
  ...
  function loadCacheInBackground(prodName) {
    ...
    switch (prodName) {
      case "JORNAIS":
        cacheFileName = "templates_jornais_cache.json";
        break;
      case "PROMO":
      ...
      default:
        cacheFileName =
          "templates_" +
          prodName.replace(/[^a-z0-9]/gi, "_").toLowerCase() +
          "_cache.json";
        break;
    }
    var templatesCacheFile = new File(
      cacheFolder.fullName + "/" + cacheFileName
    );
    if (templatesCacheFile.exists) {
      try {
        ...
        templatesCache[prodName] = [
          { type: "item", text: "Erro ao ler o cache." },
        ];
      }
    } else {
      templatesCache[prodName] = [];
  ...
  function loadTemplatesFromCache() {
    ...
    if (data.length > 0) {
      ...
      templateTree.add(
        "item",
        'Nenhum item encontrado para "' + prodName + '".'
      );
    }
    updateItemCounter();
  }
        ...
        function filterData(data) {
          ...
          for (var i = 0; i < data.length; i++) {
            ...
            if (item.type === "item") {
              ...
              if (
                typeof String.prototype.replaceSpecialCharacters === "function"
              ) {
              ...
              if (
                nodeText.indexOf(cleanSearchTerm) !== -1 ||
  ...
  D9T_TEMPLATES_w.onShow = function () {
    try {
      if (userConfigFile && userConfigFile.exists) {
        ...
        if (configContent && configContent.trim() !== "") {
          ...
          if (
            centralConfig.gnews_templates &&
            typeof centralConfig.gnews_templates.lastProductionIndex !==
              "undefined"
          ) {
            var lastIndex = parseInt(
              centralConfig.gnews_templates.lastProductionIndex
            );
            if (
              !isNaN(lastIndex) &&
              lastIndex >= 0 &&
              lastIndex < prodDrop.items.length
            ) {
    ...
    initialStatusWin.close(); // Fecha a janela de carregamento
  };
  ...
  function openTemplatesFolder() {
    ...
    if (!availablePaths || availablePaths.length === 0) {
      alert(
        "Nenhum caminho configurado para a produção '" +
    ...
    if (availablePaths.length === 1) {
      ...
      pathSelectionWin.add(
        "statictext",
        undefined,
        "Escolha um caminho para abrir:"
      );
  ...
  templateTree.onChange = function () {
    ...
    if (
      typeof this.selection.modDate !== "undefined" &&
      this.selection.modDate !== null
    ) {
    ...
    var previewBaseName =
      typeof deleteFileExt === "function"
        ? deleteFileExt(projectFile.displayName)
        : projectFile.displayName.replace(/\.[^\.]+$/, "");
    ...
    if (!foundPreview) {
      ...
      if (previewFolder.exists) {
        var previewInFolder = new File(
          previewFolder.fullName + "/" + previewBaseName + "_preview.png"
        );
        if (previewInFolder.exists) {
          previewImg.image = previewInFolder;
          foundPreview = true;
        } else {
          var potentialPreviews = previewFolder.getFiles(
            previewBaseName + ".*"
          );
  ...
  function logGNewsImport(templateName) {
    try {
      var logFolder = new Folder(
        validProductions[prodDrop.selection.index].paths[0]
      );
      var logPath = logFolder.exists
        ? logFolder.fsName
      ...
      if (typeof saveLogData === "function") {
        ...
        if (!logFile.exists) {
          logFile.open("w");
          logFile.writeln(
            "Template,Quantidade,Designer,Data,Hora,Codigo_Arte,Nome_Arte,Servidor_Destino"
          );
          logFile.close();
        }
        logFile.open("a");
