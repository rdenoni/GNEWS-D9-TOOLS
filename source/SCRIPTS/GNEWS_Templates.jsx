// =============================================================================
// GNEWS TEMPLATES - VERSÃO COM OTIMIZAÇÕES DE RENDERIZAÇÃO
// =============================================================================

function d9TemplateDialog() {
	var scriptName = 'GNEWS TEMPLATES';
	var scriptVersion = '2.8'; // Versão incrementada
                ...
                for (var i = currentBatch; i < endIndex; i++) {
                    ...
                    if (itemData.type === 'node') {
                        ...
                        if (itemData.children && itemData.children.length > 0) {
                            // CORREÇÃO: Chamando a si mesma recursivamente
    ...
    function loadTemplatesFromCache() {
        ...
        if (data && data.length > 0) {
            // Usa a nova função otimizada para não bloquear a UI
    ...
    D9T_TEMPLATES_w.onShow = function () {
        ...
        // updateArteInfo(); // Esta função não foi definida no seu código, comentei para evitar erros.
    };
	...
	if (openBtn && typeof openBtn.leftClick !== 'undefined') {
		openBtn.leftClick.onClick = function() { executeOpen(); };
	} else if (openBtn) {
		openBtn.onClick = function() { executeOpen(); };
	}
	if (importBtn && typeof importBtn.leftClick !== 'undefined') {
		importBtn.leftClick.onClick = function() { executeImport(); };
	} else if (importBtn) {
		importBtn.onClick = function() { executeImport(); };
	}
	...
	function logGNewsImport(templateName) {
		try {
			...
			var logData = [
				...
				codigoTxt.text.trim().toUpperCase(), infoValues[0].text, infoValues[1].text
			].join(',');
	...
	if (infoBtn && typeof infoBtn.leftClick !== 'undefined') {
		infoBtn.leftClick.onClick = function() { showHelpDialog(); };
	} else if (infoBtn) {
		infoBtn.onClick = function() { showHelpDialog(); };
	}

	function showHelpDialog() { /* ... implementação da ajuda ... */ }

	if (cancelBtn && typeof cancelBtn.leftClick !== 'undefined') {
		cancelBtn.leftClick.onClick = function() { D9T_TEMPLATES_w.close(); };
	} else if (cancelBtn) {
		cancelBtn.onClick = function() { D9T_TEMPLATES_w.close(); };
	}
