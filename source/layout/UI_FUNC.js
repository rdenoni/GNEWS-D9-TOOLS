...
function D9T_BUILD_UI(structureObj, uiObj) {
	...
	uiObj.prodDrop = uiObj.prodGrp.add('dropdownlist', undefined, getProdNames(D9T_prodArray));
	...
	for (var pinSec in structureObj.pinGrp) {
		...
		var pinSectionGrp = uiObj.mainGrp.add('group', undefined, { name: 'sectionGrp' });
	...
	for (var sec in structureObj.mainGrp) {
		...
		if (sectionCounter > 0) uiObj.divArray.push(new themeDivider(uiObj.mainGrp));
		var sectionGrp = uiObj.mainGrp.add('group', undefined, { name: 'sectionGrp' });
...
function D9T_UI_EVENTS(uiObj) {
    function D9T_RUN_SCRIPT(scriptFileName) {
        try {
            ...
            if (scriptFile.exists) {
                ...
                alert('Erro: O script "' + scriptFileName + '" não foi encontrado no caminho:\n' + scriptPath);
            }
        } catch (err) {
            alert('Erro ao executar o script "' + scriptFileName + '":\n' + err.message);
        }
    }
	...
	uiObj.prodDrop.onChange = function () {
		...
		if (!templatesFolder.exists) alert(lol + '#D9T_002 - pasta de templates não localizada...');
	};
...
function changeIcon(imageIndex, imagesGrp) { for (var i = 0; i < imagesGrp.children.length; i++) { imagesGrp.children[i].visible = i == imageIndex; } }
function populateMainIcons(imagesGrp) { while (imagesGrp.children.length > 0) { imagesGrp.remove(imagesGrp.children[0]); } for (var i = 0; i < D9T_prodArray.length; i++) { var newIcon = imagesGrp.add('image', undefined, undefined); try { newIcon.image = eval(D9T_prodArray[i].icon); } catch (err) { newIcon.image = defaultProductionDataObj.PRODUCTIONS[0].icon; } newIcon.helpTip = D9T_prodArray[0].name + '\n\n' + dClick + ' para editar a lista de produções'; newIcon.preferredSize = [24, 24]; newIcon.visible = i == 0; newIcon.addEventListener('click', function (c) { if (c.detail == 2) { d9ProdFoldersDialog(D9T_prodArray); D9T_ui.prodDrop.removeAll(); D9T_prodArray = updateProdData(configFile); populateDropdownList(getProdNames(D9T_prodArray), imagesGrp.parent.children[1]); populateMainIcons(imagesGrp); D9T_ui.prodDrop.selection = 0; imagesGrp.layout.layout(true); } }); } }
function themeDivider(sectionGrp) { var newDiv = sectionGrp.add('customButton', [0, 0, 1, 1]); setUiCtrlColor(newDiv, divColor1); newDiv.onDraw = customDraw; return newDiv; }
function themeIconButton(sectionGrp, ctrlProperties) { var newUiCtrlObj = {}; var tipTxt = ctrlProperties.tips.join('\n\n'); if (ctrlProperties.icon.hover == undefined) ctrlProperties.icon.hover = ctrlProperties.icon.normal; var btnGroup = sectionGrp.add('group'); var iconGroup = btnGroup.add('group'); iconGroup.orientation = 'stack'; newUiCtrlObj.leftClick = iconGroup.add('button', undefined, ''); newUiCtrlObj.leftClick.size = [0, 0]; newUiCtrlObj.leftClick.visible = false; newUiCtrlObj.rightClick = iconGroup.add('button', undefined, ''); newUiCtrlObj.rightClick.size = [0, 0]; newUiCtrlObj.rightClick.visible = false; var hoverImg = iconGroup.add('image', undefined, ctrlProperties.icon.hover); hoverImg.helpTip = tipTxt; hoverImg.visible = false; var normalImg = iconGroup.add('image', undefined, ctrlProperties.icon.normal); normalImg.helpTip = tipTxt; btnGroup.addEventListener('mouseover', function () { this.children[0].children[3].visible = false; this.children[0].children[2].visible = true; }); btnGroup.addEventListener('mouseout', function () { this.children[0].children[2].visible = false; this.children[0].children[3].visible = true; }); hoverImg.addEventListener('click', function (c) { if (c.button == 0) this.parent.children[0].notify(); if (c.button == 2) this.parent.children[1].notify(); }); return newUiCtrlObj; }
function themeImageButton(sectionGrp, ctrlProperties) { var newUiCtrlObj = {}; var newBtn = (newUiCtrlObj[ctrlProperties.key] = {}); var tipTxt = ctrlProperties.labelTxt + ':\n\n' + ctrlProperties.tips.join('\n\n'); if (ctrlProperties.icon.hover == undefined) ctrlProperties.icon.hover = ctrlProperties.icon.normal; newBtn.btnGroup = sectionGrp.add('group'); newBtn.iconGroup = newBtn.btnGroup.add('group'); newBtn.iconGroup.orientation = 'stack'; newBtn.leftClick = newBtn.iconGroup.add('button', undefined, ''); newBtn.leftClick.size = [0, 0]; newBtn.leftClick.visible = false; newBtn.rightClick = newBtn.iconGroup.add('button', undefined, ''); newBtn.rightClick.size = [0, 0]; newBtn.rightClick.visible = false; newBtn.hoverImg = newBtn.iconGroup.add('image', undefined, ctrlProperties.icon.hover); newBtn.hoverImg.helpTip = tipTxt; newBtn.hoverImg.visible = false; newBtn.normalImg = newBtn.iconGroup.add('image', undefined, ctrlProperties.icon.normal); newBtn.normalImg.helpTip = tipTxt; newBtn.label = newBtn.btnGroup.add('statictext', undefined, ctrlProperties.labelTxt, { truncate: 'end' }); newBtn.label.maximumSize = [60, 18]; newBtn.label.helpTip = tipTxt; setFgColor(newBtn.label, normalColor1); newBtn.btnGroup.addEventListener('mouseover', function () { setFgColor(this.children[1], highlightColor1); this.children[0].children[3].visible = false; this.children[0].children[2].visible = true; }); newBtn.btnGroup.addEventListener('mouseout', function () { setFgColor(this.children[1], normalColor1); this.children[0].children[2].visible = false; this.children[0].children[3].visible = true; }); newBtn.label.addEventListener('click', function (c) { if (c.button == 0) this.parent.children[0].children[0].notify(); if (c.button == 2) this.parent.children[0].children[1].notify(); }); newBtn.hoverImg.addEventListener('click', function (c) { if (c.button == 0) this.parent.children[0].notify(); if (c.button == 2) this.parent.children[1].notify(); }); return newBtn; }

// --- BOTÃO TEMA 1 "THEME BUTTON" FUNDO PRETO HOVER VERMELHO --- //
function themeButton(sectionGrp, ctrlProperties) { try { if (ctrlProperties.buttonColor === undefined) ctrlProperties.buttonColor = divColor1; if (ctrlProperties.textColor === undefined) ctrlProperties.textColor = normalColor1; var newUiCtrlObj = {}; var tipTxt = ctrlProperties.tips.join('\n\n'); var newBtnGrp = sectionGrp.add('group'); newBtnGrp.orientation = 'stack'; newUiCtrlObj.leftClick = newBtnGrp.add('button', undefined, ''); newUiCtrlObj.leftClick.size = [0, 0]; newUiCtrlObj.leftClick.visible = false; newUiCtrlObj.rightClick = newBtnGrp.add('button', undefined, ''); newUiCtrlObj.rightClick.size = [0, 0]; newUiCtrlObj.rightClick.visible = false; newUiCtrlObj.label = newBtnGrp.add('customButton'); newUiCtrlObj.label.size = [ctrlProperties.width, ctrlProperties.height]; newUiCtrlObj.label.text = ctrlProperties.labelTxt; newUiCtrlObj.label.buttonColor = hexToRgb(ctrlProperties.buttonColor); newUiCtrlObj.label.textColor = hexToRgb(ctrlProperties.textColor); newUiCtrlObj.label.minimumSize = [68, 34]; newUiCtrlObj.label.helpTip = tipTxt; drawThemeButton(newUiCtrlObj.label); newUiCtrlObj.label.addEventListener('mouseover', function () { this.textColor = [1, 1, 1, 1]; this.buttonColor = hexToRgb(highlightColor1); drawThemeButton(this); }); newUiCtrlObj.label.addEventListener('mouseout', function () { this.textColor = hexToRgb(ctrlProperties.textColor); this.buttonColor = hexToRgb(ctrlProperties.buttonColor); drawThemeButton(this); }); newUiCtrlObj.label.onClick = function () { this.parent.children[0].notify(); }; newUiCtrlObj.label.addEventListener('click', function (c) { if (c.button == 2) this.parent.children[1].notify(); }); return newUiCtrlObj; } catch (err) { alert(err.message); } }
function drawThemeButton(button) { var g = button.graphics; var textPen = g.newPen(g.PenType.SOLID_COLOR, button.textColor, 1); var fillBrush = g.newBrush(g.BrushType.SOLID_COLOR, button.buttonColor); button.onDraw = function () { var h = this.size.height; var w = this.size.width; g.ellipsePath(0, 0, h, h); g.ellipsePath(w - h, 0, h, h); g.rectPath(h / 2, 0, w - h, h); g.fillPath(fillBrush); var textLinesArray = this.text.split('\n'); var pyInc = 12; for (var l = 0; l < textLinesArray.length; l++) { var textSize = g.measureString(textLinesArray[l]); var px = (w - textSize.width) / 2; var py = l == 0 ? -(textLinesArray.length - 1) / 2 * pyInc : (py += pyInc); if (appV > 24 && l == 0) py += 8; g.drawString(textLinesArray[l], textPen, px, py); } }; }
...
function drawRoundedRect(g, brush, width, height, radius, x, y) { g.newPath(); g.ellipsePath(x, y, radius, radius); g.fillPath(brush); g.ellipsePath(width - x - radius, y, radius, radius); g.fillPath(brush); g.ellipsePath(width - x - radius, height - y - radius, radius, radius); g.fillPath(brush); g.ellipsePath(x, height - y - radius, radius, radius); g.fillPath(brush); g.newPath(); var coords = [x, y + radius / 2, x + radius / 2, y, width - x - radius / 2, y, width - x, y + radius / 2, width - x, height - y - radius / 2, width - x - radius / 2, height - y, x + radius / 2, height - y, x, height - y - radius / 2]; for (var i = 0; i <= coords.length - 1; i += 2) { if (i == 0) { g.moveTo(coords[i], coords[i + 1]); } else { g.lineTo(coords[i], coords[i + 1]); } } g.fillPath(brush); }
function setBgColor(w, hex) { var color = hexToRgb(hex); var bType = w.graphics.BrushType.SOLID_COLOR; w.graphics.backgroundColor = w.graphics.newBrush(bType, color); }
function setUiCtrlColor(ctrl, hex) { var color = hexToRgb(hex); var bType = ctrl.graphics.BrushType.SOLID_COLOR; ctrl.fillBrush = ctrl.graphics.newBrush(bType, color); }
function setFgColor(ctrl, hex) { var color = hexToRgb(hex); var pType = ctrl.graphics.PenType.SOLID_COLOR; ctrl.graphics.foregroundColor = ctrl.graphics.newPen(pType, color, 1); }
function setCtrlHighlight(ctrl, normalColor1, highlightColor1) { setFgColor(ctrl, normalColor1); ctrl.addEventListener('mouseover', function () { setFgColor(ctrl, highlightColor1); }); ctrl.addEventListener('mouseout', function () { setFgColor(ctrl, normalColor1); }); }