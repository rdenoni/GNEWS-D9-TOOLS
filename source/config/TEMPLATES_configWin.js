...
function createStatusWindow(title) {
  ...
  win.update = function (statusText, count) {
    ...
    if (count !== undefined)
      progressLabel.text = count + " arquivos processados";
...
function getFolderStructureAsData(
  rootFolder,
  fileFilter,
  categoryName,
  progress
  ...
  try {
    ...
    writeLn(
      "Aviso: Não foi possível ler o conteúdo da pasta (verifique permissões): " +
        decodeURI(rootFolder.fsName)
    );
    return [];
  }
  if (allItems === null) {
    writeLn(
      "Aviso: O conteúdo da pasta retornou nulo (pode estar vazia ou inacessível): " +
        decodeURI(rootFolder.fsName)
    );
    return [];
  }
  var folders = [];
  var files = [];
  // ALTERAÇÃO: Lista de exclusão completa e em maiúsculas.
  var jornaisExclusions = [
    "ICONES",
    "ILUSTRACOES",
    "FOTOS PARA ABERTURAS",
    "BAGUNCA ALHEIA",
    "_OLD",
    "BACKUP",
    "VERSOES ANTERIORES",
  ...
  for (var i = 0; i < allItems.length; i++) {
    ...
    if (item instanceof Folder) {
      // ALTERAÇÃO: Verificação case-insensitive.
      var upperCaseDisplayName = item.displayName.toUpperCase();
      if (
        upperCaseDisplayName === "ADOBE AFTER EFFECTS AUTO-SAVE" ||
        upperCaseDisplayName.slice(-4) === "_AME"
      )
        continue;
      if (
        categoryName === "JORNAIS" &&
        jornaisExclusions.indexOf(upperCaseDisplayName) > -1
      )
        continue;
      var subItems = getFolderStructureAsData(
        item,
        fileFilter,
        categoryName,
        progress
      ...
      if (
        item.displayName.toLowerCase().indexOf("auto-save") > -1 ||
        item.displayName.slice(0, 7) === "tmpAEto"
      )
        continue;
...
function d9ProdFoldersDialog(prodArray) {
  ...
  var D9T_CONFIG_w = new Window(
    "dialog",
    scriptName +
      (typeof scriptVersion !== "undefined" ? " " + scriptVersion : "")
  );
  ...
  var listLabTxt = headerGrp.add(
    "statictext",
    undefined,
    "CONFIGURAÇÃO DE CAMINHOS:"
  );
  try {
    setFgColor(listLabTxt, normalColor1);
  } catch (e) {}
  var mainGrp = D9T_CONFIG_w.add("group", undefined);
  mainGrp.orientation = "column";
  mainGrp.spacing = 16;
  var pathLabsToCheck = []; // Array para verificação assíncrona

  function getCacheFileName(categoryName) {
    switch (categoryName) {
      case "JORNAIS":
        return "templates_jornais_cache.json";
      case "PROMO":
        return "templates_promo_cache.json";
      case "PROGRAMAS":
        return "templates_programas_cache.json";
      case "EVENTOS":
        return "templates_eventos_cache.json";
      case "MARKETING":
        return "templates_marketing_cache.json";
      case "BASE TEMÁTICA":
        return "templates_base_cache.json";
      case "ILUSTRAÇÕES":
        return "templates_ilustra_cache.json";
      default:
        return (
          "templates_" +
          categoryName.replace(/[^a-z0-9]/gi, "_").toLowerCase() +
          "_cache.json"
        );
    }
  }
  function checkPathStatus(pathStr, categoryName) {
    ...
    var cacheFile = new File(
      cacheFolder.fullName + "/" + getCacheFileName(categoryName)
    );
    if (!cacheFile.exists) return "red";
    try {
      cacheFile.open("r");
      var content = cacheFile.read();
      cacheFile.close();
      if (!content) return "red";
      var cacheData = JSON.parse(content);
      if (cacheData && cacheData.hasOwnProperty(pathStr)) return "#2E8B57";
    } catch (e) {
      return "red";
    }
    return "red";
  ...
  for (var c = 0; c < categorias.length; c++) {
    ...
    try {
      setFgColor(catLab, normalColor1);
    } catch (e) {}
    var catAddBtn;
    try {
      catAddBtn = new themeIconButton(catHeaderGrp, {
        icon: D9T_MAIS_ICON,
        tips: [lClick + "adicionar caminho"],
      });
    } catch (e) {
      catAddBtn = catHeaderGrp.add("button", undefined, "+");
  ...
  function addPathLine(parentGrp, pathTxt, categoryName) {
    ...
    var openBtn;
    try {
      openBtn = new themeIconButton(pathLineGrp, {
        icon: D9T_PASTA_ICON,
        tips: [lClick + "selecionar pasta"],
      });
    } catch (e) {
      openBtn = pathLineGrp.add("button", undefined, "📁");
      openBtn.preferredSize = [24, 24];
      openBtn.helpTip = "selecionar pasta";
    }
    var pathLab = pathLineGrp.add("statictext", undefined, pathTxt, {
      pathValue: pathTxt,
      truncate: "middle",
    });
    ...
    try {
      setFgColor(pathLab, normalColor2);
    } catch (e) {} // Cor neutra inicial
    pathLabsToCheck.push({ label: pathLab, category: categoryName }); // Adiciona à fila de verificação
    var testBtn = pathLineGrp.add("button", undefined, "Testar e Gerar Cache");
    testBtn.preferredSize = [120, 24];
    var deletePathBtn;
    try {
      deletePathBtn = new themeIconButton(pathLineGrp, {
        icon: D9T_FECHAR_ICON,
        tips: [lClick + "deletar caminho"],
      });
    } catch (e) {
      deletePathBtn = pathLineGrp.add("button", undefined, "X");
    ...
    testBtn.onClick = function () {
      ...
      try {
        var treeData = getFolderStructureAsData(
          folder,
          fileFilter,
          categoryName,
          progress
  ...
  function createButton(parent, config) {
    var btn;
    try {
      btn = new themeButton(parent, config);
    } catch (e) {
      btn = parent.add("button", undefined, config.labelTxt);
      btn.preferredSize = [config.width, config.height];
      btn.helpTip = config.tips[0] || "";
    }
    return btn;
  }
  var importBtn = createButton(bGrp1, {
    width: 80,
    height: 32,
    labelTxt: "importar",
    tips: ["importar configuração"],
  });
  var exportBtn = createButton(bGrp1, {
    width: 80,
    height: 32,
    labelTxt: "exportar",
    tips: ["exportar configuração"],
  });
  var saveBtn = createButton(bGrp2, {
    width: 120,
    height: 32,
    labelTxt: "salvar",
    tips: ["salvar configuração"],
  });
  try {
    setBgColor(D9T_CONFIG_w, bgColor1);
  } catch (e) {}
  D9T_CONFIG_w.onShow = function () {
    // Verificação assíncrona dos caminhos após a janela ser exibida.
    for (var i = 0; i < pathLabsToCheck.length; i++) {
      var item = pathLabsToCheck[i];
      var color = checkPathStatus(
        item.label.properties.pathValue,
        item.category
      );
      setFgColor(item.label, color);
    }
  };
  function setupButtonClick(btn, func) {
    if (btn.leftClick) {
  ...
  function saveProdData(dataToSave) {
    var configFile = new File(
      scriptMainPath + "source/config/TEMPLATES_config.json"
    );
    try {
  ...
  function repopulateUI(configData) {
    ...
    D9T_CONFIG_w.layout.layout(true);
    D9T_CONFIG_w.onShow(); // Re-executa a verificação de cor para os novos caminhos
  }
  setupButtonClick(saveBtn, function () {
    try {
      ...
      alert("Erro ao coletar dados para salvar: " + err.message);
    }
  });
  setupButtonClick(exportBtn, function () {
    ...
    if (saveFile) {
      try {
        ...
        saveFile.close();
        alert(
          "Configuração exportada com sucesso para:\n" +
  ...
  setupButtonClick(importBtn, function () {
    var configFile = File.openDialog(
      "Selecione um arquivo de configuração (.json)",
      "*.json"
    );
    if (configFile) {
      try {
  ...
  function collectConfigData() {
    ...
    for (var i = 0; i < categorias.length; i++) {
      ...
      if (cat.uiGroup && cat.uiGroup.children) {
        for (var j = 0; j < cat.uiGroup.children.length; j++) {
          caminhos.push(
            cat.uiGroup.children[j].children[1].properties.pathValue
          );
        }
      }
    ...
    configOutput.icon = "";
    configOutput.templatesPath = configOutput.jornais
